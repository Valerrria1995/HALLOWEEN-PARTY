...
function preload() {
  spiderImage = loadImage('spider-1.png'); //תמונה של העכביש שעולה
   face = loadImage('face.png'); //תמונה של הדלעת ברקע
  img = loadImage('beak.png'); // קורי עכביש בצד ימין
  candyImage = loadImage('candy.png'); //סוכריה
  font = loadFont('Noot.otf'); 
...
function draw() {
  background(random (0,15));
   imageMode(CENTER);
  image(witchImage, mouseX, mouseY, 100, 100);
   noStroke();
  rectMode(CENTER);
